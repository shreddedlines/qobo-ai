...
/** A numbered piece of evidence shown to the model as <source id="S1">. */
export interface ContextSource {
  id: string;
  title: string;
  url: string;
  text: string;
  origin: 'kb' | 'discrepancy';
}
...
export function buildContextSources(chunks: RetrievedChunk[], discrepancies: Discrepancy[]): ContextSource[] {
  ...
  for (const statement of discrepancies.flatMap((discrepancy) => discrepancy.statements)) {
    ...
    seen.add(key);
    sources.push({ id: `S${sources.length + 1}`, title: statement.title, url: statement.url, text: statement.quote, origin: 'discrepancy' });
  }
  return sources;
}
...
const MARKER_GROUP = /\[\s*(S\d+(?:\s*[,;]\s*S\d+)*)\s*\]/g;
...
export function resolveCitations(answer: string, declaredIds: string[], contextSources: ContextSource[], nonCitableIds: string[] = []): ResolvedCitations {
  ...
  const numberFor = (source: ContextSource): number => {
    ...
    if (number === undefined) {
      ...
      sources.push({ title: source.title, url: source.url, kind: 'qobo' });
    }
    return number;
  };

  let content = answer.replace(MARKER_GROUP, (_match, group: string) => {
    const numbers = [
      ...new Set(
        group
          .split(/[,;]/)
